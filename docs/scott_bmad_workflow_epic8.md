# BMAD Workflow Guide - Epic 8 and Beyond

**Author:** Bob (Scrum Master)  
**Date:** November 17, 2025  
**For:** Scott E. Townsend  
**Purpose:** Strict BMAD discipline to prevent workflow drift and architectural violations

---

## Situation Assessment

### What We Have (Code in Production)

**Completed Epics (Code Exists, Works):**
- ✅ Epic 1: Foundation & Project Init
- ✅ Epic 2: Design System (documented)
- ✅ Epic 3: Layout & Navigation
- ✅ Epic 4: Homepage
- ✅ Epic 5: Product Pages
- ✅ Epic 6: Pricing System
- ✅ Epic 7: Support Wiki (Story 7.1 complete)

**Total Stories Implemented:** ~30 stories  
**Code Quality:** Functional, architecture-compliant  
**CI/CD:** Passing

### What We're Missing (BMAD Artifacts)

**Missing Documentation:**
- ❌ Epic summaries (only 2 of 7 exist)
- ❌ Story files (none in `.bmad-ephemeral/stories/`)
- ❌ Story context XML files
- ❌ Sprint status tracking (deleted during revert)
- ❌ Dev completion records for most stories

**Problem Started:** After Epic 2 (November 12, 2025)  
**Result:** 8 reverts in 5 days due to workflow drift

---

## Recovery Decision: Clean Break

**What We're NOT Doing:**
- ❌ Retroactive documentation of Epics 3-7
- ❌ Backfilling story files for completed work
- ❌ Creating post-facto context XML

**What We ARE Doing:**
- ✅ Draw line at Epic 7
- ✅ Acknowledge past work exists and functions
- ✅ **Starting Epic 8:** Full BMAD discipline
- ✅ Learn from mistakes, move forward

**Rationale:** Code works. Time better spent on future quality than expensive retroactive docs.

---

## The Strict BMAD Workflow (Non-Negotiable)
...
STEP 1: Sprint Planning (once per epic)
   @sm.md *sprint-planning
   → Generates/updates sprint-status.yaml
   → Shows all stories and their states

STEP 2: For Each Story (8.1, then 8.2)
   ├─ @sm.md *create-story
   │  └─ Status: drafted
   │
   ├─ @sm.md *story-context
   ...
   │
   ├─ (Optional) @sm.md *code-review
   │  └─ Status: review → done
   │
   └─ Create story summary doc manually

STEP 3: Epic Complete
   ├─ Create EPIC-8-SUMMARY.md manually
   ├─ (Optional) @sm.md *epic-retrospective
   ├─ Commit all changes
   ├─ Push to remote
   ├─ Verify CI/CD passes
...
## Story-Level Workflow (For Each Story)

### Story 8.1: Create Use Case Layout

#### Command 1: Create Story Draft
```
@sm.md *create-story
```

**What Happens:**
- SM reads `docs/technical/epic-story-task-breakdown.md`
- Finds first "backlog" story in Epic 8 (Story 8.1)
- Extracts acceptance criteria from epic breakdown
- Loads `docs/technical/architecture-decisions.md` for constraints
- Creates story file: `.bmad-ephemeral/stories/8-1-create-use-case-layout.md`
- Updates sprint-status: `8-1-create-use-case-layout: backlog → drafted`
...
Story ID: 8.1
Story Key: 8-1-create-use-case-layout
File: .bmad-ephemeral/stories/8-1-create-use-case-layout.md
Status: drafted (was backlog)

Next Steps:
1. Review the drafted story
2. Run *story-context to generate technical context
```

**You Review:** 
- Read `.bmad-ephemeral/stories/8-1-create-use-case-layout.md`
- Verify ACs match epic breakdown
- Check tasks are actionable

---

#### Command 2: Generate Technical Context
```
@sm.md *story-context
```

**What Happens:**
- SM loads the drafted story file
- Loads `docs/technical/architecture-decisions.md` (React patterns, component rules)
- Reviews Story 7.1 completion summary for learnings
- Analyzes:
  - Server vs client component boundaries
  - Global scope impact (will this affect other routes?)
  - Reusable components from previous stories
  - React serialization rules
  - Build and test checkpoints
- Creates context XML: `.bmad-ephemeral/stories/8-1-create-use-case-layout-context.xml`
- Updates story file: Populates "Context Reference" section with XML path
- Updates sprint-status: `8-1-create-use-case-layout: drafted → ready-for-dev`

**SM Reports:**
```
✅ Story Context Generated!

Context File: .bmad-ephemeral/stories/8-1-create-use-case-layout-context.xml
Story Status: ready-for-dev (was drafted)

Story is now ready for Dev Agent.

Next Step: @dev.md *develop-story
```

**You Review:**
- Skim context XML for architectural guidance
- Verify references to architecture patterns

---

#### Command 3: Implement Story
```
@dev.md *develop-story
```
...
1. ✅ Loads story file
2. ✅ Checks status = "ready-for-dev" or "Approved"
3. ✅ Checks Context Reference section populated
4. ✅ If either fails → HALT with error

**If Gates Pass:**
- Loads context XML files
- Reviews architectural guidance
- Reviews Story 7.1 learnings (what patterns exist, what to reuse)
- Implements each task systematically
- **Builds after first component** (catches issues early)
- Tests on multiple breakpoints
- Updates story file: Dev Agent Record sections
- Updates sprint-status: `8-1-create-use-case-layout: ready-for-dev → review`
...
- src/components/use-cases/use-case-card.tsx
- src/components/use-cases/index.ts

All ACs satisfied. Build passing. Tests passing.

Status: review (ready for code review)

Next Step: @sm.md *code-review or merge if approved
```
...
#### Command 4: Code Review (Optional)
```
@sm.md *code-review
```

**What Happens:**
- SM reviews implementation against ACs
- Checks architecture-decisions.md compliance
- Validates no global scope violations
- Validates React server/client boundaries correct
- Creates review section in story file
- Updates sprint-status: `8-1-create-use-case-layout: review → done`
...
Story 8.1 is complete. Ready for next story.
```

---

#### Command 5: Create Story Summary (Manual)
```
(No workflow command - manual documentation)
```

**You Create:** `docs/EPIC-8-STORY-8.1-SUMMARY.md`
...
**Reference:** Use `docs/EPIC-7-STORY-7.1-SUMMARY.md` as template

---

### Story 8.2: Add Beta Tester Placeholder Sections

**Repeat Commands 1-5** for Story 8.2:
...
After all stories in Epic 8 complete:

- [ ] **Epic Summary Doc:** Create `docs/technical/EPIC-8-SUMMARY.md`
  - List all completed stories (8.1, 8.2)
  - Document files created
  - Note patterns established
  - Capture lessons learned
  
- [ ] **Optional Retrospective:** Run `@sm.md *epic-retrospective`
  - Review epic process
  - Identify improvements
  - Document for next epic

- [ ] **Git Workflow:**
  - Commit all changes on `epic/8-use-cases` branch
  - Push to remote
  - Verify CI/CD passes
  - Merge to main
  - Delete epic branch

- [ ] **Sprint Status Update:** Mark epic-8 as "done" in sprint-status.yaml

---

## Critical Discipline Rules

### Rule 1: Always Mention Agent

**Every single prompt must start with agent mention:**
...
**Why:** Without `@agent` mention, regular Cursor AI responds (no BMAD structure, no gates, no discipline).

---

### Rule 2: Always Use Workflow Commands

**Use asterisk (*) commands from agent menus:**
...
❌ **INCORRECT:**
```
@sm.md create the next story
@sm.md generate context for story 8.1
@dev.md implement story 8.1
```

**Why:** Workflow commands trigger specific BMAD workflows with gates and validation. Natural language bypasses the workflow engine.

---

### Rule 3: One Workflow Per Prompt

**Do NOT chain commands:**

✅ **CORRECT:**
```
Prompt 1: @sm.md *create-story
(Wait for completion)

Prompt 2: @sm.md *story-context
(Wait for completion)

Prompt 3: @dev.md *develop-story
```

❌ **INCORRECT:**
```
@sm.md *create-story then *story-context then implement
```

**Why:** Each workflow is a distinct operation. Chaining can cause workflow engine confusion or bypasses.

---

### Rule 4: Wait for Workflow Completion

**After each command:**
1. Agent executes workflow
2. Agent reports completion
3. You review output
4. You issue next command
...
**Each workflow ends with a report. Read it. Verify. Then proceed.**

---

### Rule 5: Verify Gates Pass

**Before Dev Agent starts coding:**

Check story file has:
- Status: "ready-for-dev" or "Approved" (not "drafted")
- Context Reference: File path(s) to XML (not empty `<!--` comment)

**If either is empty → Dev will HALT. This is correct behavior.**
...
| Command | Agent | Purpose | When to Use |
|---------|-------|---------|-------------|
| `*sprint-planning` | @sm.md | Generate/update sprint-status.yaml | Start of epic or when tracking lost |
| `*workflow-status` | @sm.md | Check current workflow state | When unsure what to do next |
| `*epic-retrospective` | @sm.md | Review completed epic | After all epic stories done |

### Story Preparation Commands

| Command | Agent | Purpose | When to Use |
|---------|-------|---------|-------------|
| `*create-story` | @sm.md | Create next backlog story | Starting new story |
| `*story-context` | @sm.md | Generate technical context XML | After story drafted |
| `*story-ready` | @sm.md | Mark story ready without context | Only if manually reviewing |

### Development Commands

| Command | Agent | Purpose | When to Use |
|---------|-------|---------|-------------|
| `*develop-story` | @dev.md | Implement story with context | After story-context complete |
| `*code-review` | @sm.md | Review completed story | After dev implementation |
| `*story-done` | @dev.md | Mark story done | After all ACs satisfied |
